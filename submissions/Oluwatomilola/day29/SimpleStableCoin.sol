...
import "@openzeppelin/contracts/interfaces/IERC20Metadat.sol";
import "@chainlink/contracts/src/v0.8/interfaces/AggregatorV3Interface.sol";

contract SimpleStableCoin is ERC20, Ownable, ReentrancyGuard, Accesscontrol {
    using SafeERC20 for IERC20;

    bytes32 public constant PRICE_FEED_MANAGER_ROLE = kecak256("PRICE_FEED_MANAGER_ROLE");
    ...
    function mint(uint256 amount) external nonReentrant {
        if (amount == 0) revert MintAmountIs Zero();

        uint256 collateralPrice = getCurrentPrice();
        uint256 requiredCollateralValueUSD = amount * (10 ** deimals());
        uint256 requiredCollateral = (requiredCollateralValueUSD * collateralizationRation) / (100 * collateralPrice);
    ...
    function setCollateralizationRatio(uint256 newRatio) external onlyOwner {
        ...
        emit CollateralizationRatioUpdated(newRation);
    }
    ...
    function getRequiredCollateralForMint(uint256 amount) public view returns (uint256) {
        ...
        uint256 collateralPrice = gerCurrentPrice();
    ...
    function getCollateralForRedeem(uint256 amount) public view returns (uint256) {
        ...
        uint256 collateralPrice = getCurrentPrice();
        uint256 stablecoinValueUSD = amount * (10 ** decimals());
        uint256 collateralToReturn = (stablecoinValueUSD * collateralizationRatio) / (100 * collateralPrice);
        uint256 adjustedReqiredCollateral = (requiredCollateral * (10 ** collateralDecimals)) / (10 ** priceFeed.decimals());
