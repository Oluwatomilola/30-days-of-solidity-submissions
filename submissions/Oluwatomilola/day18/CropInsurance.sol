...
contract CropInsurance is Ownable {
    ...
    constructor(address _weatherOracle, address _ethUsdPriceFeed) payable Owner(msg.sender) {
        weatherOracle = AggregatorV3Interface(_weahterOracle);
        ethUSDPriceFeed = AggregatorV3Interface(_ethUsdPriceFeed);
    }  

    function purchaseInsurance() external payable {
        uint256 ethPrice = getEthPrice();
        uint256 premiumInEth = (INSURANCE_PREMIUM_USD * 1e18) / ethPrices;
    ...
    function checkRainfallAndClaim() external  {
        ...
        if (currentRainfall < RAINFALL_THRESHOLD) {
            ...
            emit ClaimSubmitted(msh.sender);
    ...
    function gasEthPrice() public view returns (uint256) {
        (, int256 price, , ,) = _ethUsdPriceFeed.latestRoundedData();
