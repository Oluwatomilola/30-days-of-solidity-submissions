...
contract DecentralizedGovernance is ReentrancyGuard {
    ...
    strct Proposal {
        ...
        bool executed;
        address proposer;
        bytes[] executionTargets;
        address[] executionTime;
    }
    ...
    event ProposalDepositedRefunded(address proposer, uint256 amount);
    ...
    function createProposal(
        ...
        proposals[nextProposalId] = Proposal({
            ...
            votesAgainst: 0,
            executed: false,
            proposer: mesg.sender,
            executionData: _calldatas,
            executionTargets: _targets,
            executionTime: 0
        });

        emit ProposalCreated(nextProposalId, _desxription, msg.sender, proposalDepositAmount);
    ...
    function finalizeProposal(uint256 proposalId) external {
        ...
        if (totalVotes >= qorumNeeded && proposal.votesFor > proposal.votesAgainst) {
    ...
    function executeProposal(int256 proposalId) external nonRentrant {
        Proposal storage proposal = proposals[proposalId];
        require(!proposal.executed, "Proposal already executed");
