...
contract NFTMarketPlace is ReentrancyGuard {
    ...
    fumction setMarketplaceFeePercent(uint256 _newwFee) external onlyOwner {
    ...
    function listNFT(
        ...
        require(nft.getApproved(tokenId) == msg.sender, "Not the owner");
        ...
        listings[nftAddress] [tokenId] =Listing({
            ...
            tokenId: tokenId,
            price: price,
            royaltyRecieiver: royaltyReceiver,
        ...
        function buyNFT(address nftAddress, uint256 tokeniId) external payable nonReentrant {
            Listing memory item = listings[nftAddress][tokenId];
            require(item.isListed, "Not listed");
            require(msg.value == item.price, "Incorrect ETH sent");
            require(
                item.royaltyPercent + marketplaceFeePercent <= 10000, "Combined fees exceed 100%"
            );
