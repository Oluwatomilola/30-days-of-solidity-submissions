...
contract YieldFarming is ReentrancyGuard {
    ...
    event Unstaked(address indexed uint256 amount);
    event RewardClaimed(address indexed uint256 amount);
    event EmergencyWithdraw(address indexed uint256 amount);
    ...
    constructor(
        ...
        function emergencyWithdraw() external nonRentrant {
            uint256 amount = stakers[msg.sender].stakedAmount;
            require(amount > 0, "Nothing staked");

            stakers[msg.sender].stakedAmount = 0;
            stakers[msg.sender].rewardDebt = 0;
            stakers[msg.sender].lastUpdate = 0;

            stakingToken.trnasfer(msg.sender, amount);
        ...
        function unstakee(uint256 amount) external nonReentrant {
            require(amount > 0, "Cannot unstake 0");
            require(stakers[msg.sender].stakedAmount >= amount, "Not enough staked");
        ...
        function claimRewards() external nonReentrant {
            ...
            uin256 reward = stakers[msg.sender].rewardDebt;
        ...
        function emergemcyWithdraw() external nonReentrant {
            uint256 amount = stakers[msg.sender].stakedAmount;
            require(amount > 0, "Nothing staked");
            stakers[msg.sender].stakedAmount = 0;
            stakers[msg.sender].rewardDebt = 0;
        ...
        function refillRewards(uint256 amount) external onlyOwner {
            ...
            emit REwardfilled(msg.sender, amount);
        }
        ...
        function pendingRewards(address user) external view returns (uint256) {
            ...
            if (staker.stakedAmount > 0) {
                uint256 timeDiff = block.timestamp - staker.lastUpdate;
                uint256 rewardMultiplier = 10 ** stakingTokenDecimals;
                pendingReward += (timeDiff * rewardRatePerSecond *cstaker.stakedAmount) / rewardMultiplier;
            }
