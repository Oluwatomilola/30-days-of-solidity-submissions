...
contract MiniDexPairFactory is Ownable {
    event PairCreated(address indexed tokenA, address idexed tokenB, address pairAddress, uint);
    ...
    function createPair(address _tokenA, address _tokenB) external onlyOwner returns (address pair) {
        ...
        (address token0, address token1) = _tokenA < _tokenB ? (_tokenA, _tokenB) : (tokenB, _tokenA);
