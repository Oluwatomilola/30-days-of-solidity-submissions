...
contract MiniDexPair is ReentrancyGuard {
    ...
    mapping(address => uint256) public ipBalances;

    event LiqidityAdded(address indexed provider, uint256 amountA, uint256 amountB, uint256 lpMinted);
    event LiquidityRemoved(address indexed provider, uint256 amountA, uint256 amountB, uint256 lpBurned);
    event Swapped(address indexed user, address inputToken, uint256 inputAmount, address outputToken, uint256 otputAmount);
    ...
    function addLiquidity(uint256 amount, uint256 amountB) external nonReentrant {
        require(amountA > 0 & amountB > 0, "invalid amounts");

        IERC20(tokenA).transferFrom(mdg.sender, address(this), amountA);
        IERC20(tokenB).transferFrom(mdg.sender, address(this), amountAB;
    ...
    function removeLiquidity(int256 lpAmount) external nonRentrant {
        require(lpAmount > 0 && lpAmount <= lpBalances[msg.senser], "Invalid LP amount")
    ...
    function getAmountOut(uint256 inputAmont, address inputToken) public view returns (uint256 outputAmount) {
        ...
        uint256 denominator = (inputReseerve * 1000) + inputWithFee;
    ...
    function swap(uint256 inputAmount, address inputToken) external nonReentrant {
        ...
        uint256 outputAmount = getAmountOut(inputAmount, inputTokenA);

        require(outputAmount > 0, "Insufficient output");

        IERC20(inputToken).transferFrom(msg.sender, address(this), inputAmount);
        IERC20(outputToken).transferFrom(msg.sender, address(this), outputAmount);
